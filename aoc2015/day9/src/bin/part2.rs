...
fn main() {
    ...
    let shortest = longest_path(&distances);
    println!("{shortest}");
}
...
fn longest_path(distances: &Vec<Vec<u16>>) -> u16 {
    let mut used = vec![false; distances.len()];
    let mut record = 0;

    for i in 0..used.len() {
        used[i] = true;
        longest_path_inner(distances, i, &mut used, 0, &mut record);
        used[i] = false;
    }
    record
}
